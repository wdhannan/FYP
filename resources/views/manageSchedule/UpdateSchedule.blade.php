...
@section('content')
    ...
    <div class="page-wrapper">
        <div class="page-header">
            <h1 class="page-title">✏️ Update Schedule</h1>
        </div>
        ...
        <div class="schedule-form-wrapper">
            @php
                ...
                if (isset($schedule) && $schedule) {
                    // Try different ways to access ScheduleID
                    $scheduleIdValue = $schedule->ScheduleID ?? 
                                     (is_object($schedule) ? ($schedule->{'ScheduleID'} ?? null) : null) ??
                                     (is_array($schedule) ? ($schedule['ScheduleID'] ?? null) : null);
                }
            @endphp
            
            @if(!isset($schedule) || !$schedule || !$scheduleIdValue)
                <div style="padding: 20px; background: #f8d7da; color: #721c24; border-radius: 8px; margin-bottom: 20px;">
                    <strong>Error:</strong> Schedule not found or invalid ScheduleID. Please go back and try again.
                    @if(isset($schedule))
                        <br><small>Debug Info: Schedule object exists. ScheduleID value: {{ var_export($scheduleIdValue, true) }}</small>
                        <br><small>Schedule object type: {{ gettype($schedule) }}</small>
                        @if(is_object($schedule))
                            <br><small>Schedule properties: {{ implode(', ', array_keys((array)$schedule)) }}</small>
                        @endif
                    @endif
                </div>
            @else
            <form id="updateScheduleForm" action="{{ route('schedule.update', ['scheduleId' => $scheduleIdValue]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                
                <div class="form-section">
                    <label class="form-label">Schedule ID</label>
                    <input type="text" class="form-input" value="{{ $scheduleIdValue ?? 'N/A' }}" readonly>
                </div>

                <div class="form-section">
                    <label class="form-label" for="upload_date">Schedule Date</label>
                    <input type="date" id="upload_date" name="upload_date" class="form-input" value="{{ old('upload_date', isset($schedule->UploadDate) && $schedule->UploadDate ? \Carbon\Carbon::parse($schedule->UploadDate)->format('Y-m-d') : '') }}" required>
                </div>

                <div class="form-section">
                    <label class="form-label">Current File</label>
                    <div class="current-file">
                        📄 {{ $schedule->FileName ?? 'No file' }}
                    </div>
                </div>

                <div class="form-section">
                    <label class="form-label">New Schedule File (Optional)</label>
                    <div class="file-upload-section">
                        <div class="file-input-wrapper">
                            <input type="text" class="file-input" id="fileDisplay" placeholder="No file chosen (keep current file)" readonly>
                            <label for="schedule_file" class="upload-icon-btn">
                                <svg class="upload-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path>
                                </svg>
                            </label>
                            <input type="file" id="schedule_file" name="schedule_file" accept=".png,.jpg,.jpeg,image/png,image/jpeg" style="display:none">
                        </div>
                ...
                <div class="form-actions">
                    <a href="{{ route('schedule.index') }}" class="cancel-btn">Cancel</a>
                    <button type="submit" class="submit-btn" id="submitBtn">Confirm</button>
                </div>
            </form>
            @endif
        </div>
    ...
    <script>
        ...
        fileInput.addEventListener('change', function(event) {
            ...
            if (file) {
                ...
                fileNameDisplay.textContent = `New file: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`;
